...
function mainfunction(){
	...
	$version=readfile1("$rootdir/userdata/$handle/$jobid/output/version");
	$currentversion=readfile1("$rootdir/PROG/pomoscan/version");
	if ($version!=""){
	...
	if ($version<$currentversion){
		echo "< $currentversion. You may want to resubmit the job by editing the job from the view jobs page.<br><br>";
	}else echo "<br><br>";
...
if (!$downloadastext){
	$i=0;
	echo "<br><a href='../Centipede_Scan/submit.php?pipeline=1&email=$email&handle=$handle&jobid=$jobid'>send job to Centipede</a>";
	echo "<br><a href='../Motif_Scan/submit.php?pipeline=1&email=$email&handle=$handle&jobid=$jobid'>send job to Motif_Scan</a>";
	...
	echo "<tr align=center>";
		...
		echo "<td>P-value</td>";
	echo "</tr>";
	$topmotifFP=fopen("$rootdir/userdata/$handle/$jobid/output/topmotif",'w');	
	foreach ($table as $family=>$row){
		...
		$count1=readfile1("$rootdir/userdata/$handle/$jobid/output/$name.count1");
		$count2=readfile1("$rootdir/userdata/$handle/$jobid/output/$name.count2");
		$score=$row["SCORE"];
		$w=$row["W"];
		...
		echo "<td>";
		if ($showmode=="family"){
			echo "<a href='view.php?family=$family&top=999&show=factor&email=$email&handle=$handle&jobid=$jobid'>$family</a>";
		}
		else{
			$isfirst=1;
	...
	echo "</table>";
	fclose($topmotifFP);
	chmod("$rootdir/userdata/$handle/$jobid/output/topmotif",644);
}
else{
	...
	foreach ($table as $family=>$row){
		...
		$movingaverage=ahrefimage("$resultLink/$name.pos.movingaverage.jpg","$resultLink/$name.pos.movingaverage.jpg",100);
		$movingaverage2=ahrefimage("$resultLink/$name.pos.movingaverage2.jpg","$resultLink/$name.pos.movingaverage2.jpg",100);
		$count1=trim(readfile1("$rootdir/userdata/$handle/$jobid/output/$name.count1"));
		$count2=trim(readfile1("$rootdir/userdata/$handle/$jobid/output/$name.count2"));
		$score=$row["SCORE"];
		$w=$row["W"];
		$threshold=$row["THRESHOLD"];
		$z0score=$row["Z0SCORE"];
