...
function mainfunction(){
	...
	$jobdir="$rootdir/userdata/$handle/$jobid";
       //  $rootdir="/home/chipseq/public_html/";
        ...
        $resultLink=str_replace("/var/www/html","http://plap11",$rundir);
	$runDir=$rundir;
...
if (!$downloadastext){
	$i=0;
	echo "<br><a href='../Centipede_Scan/submit.php?pipeline=1&rundir=$rundir'>send job to Centipede</a>";
	echo "<form>";
	if ($showmode=="family"){
	...
	foreach ($table as $family=>$row){
		...
		echo "<td>";
		if ($showmode=="family"){
			echo "<a href='viewresult.php?family=$family&top=999&show=factor&rundir=$rundir'>$family</a>";
		}
		else{
			$isfirst=1;
