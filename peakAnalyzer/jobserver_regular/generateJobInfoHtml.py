...
def getRunInfo(npeaks,fname):
    ...
    for j in xrange((i+1),len(f)):
        if f[j].startswith("#") and j!=(i+4): #omit max dup tags at same pos info
            tmp=f[j]
...
def generatePkCallingStats(result_dir):
    ...
    macs_version='1.4.2'
    macs_info='<style> #table_macs td{text-align:center;width:50%}</style> <table id="table_macs" class="table table-bordered table-condensed"><tr><td colspan="2"><strong>MACS Arguments</strong></td></tr>\
