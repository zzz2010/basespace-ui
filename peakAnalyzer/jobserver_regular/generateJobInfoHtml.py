...
def generateMappingStats(result_dir):
    ...
    pcrfilterfile=glob.glob(pkcall_outdir+"*.unique")
    if  maplogfiles:
            ...
            for i in xrange(len(maplogfiles)):
                ...
                num_uniq=numreads[2]
                num_mm=numreads[3]
                fname=os.path.basename(maplogfiles[i])
    ...
    try:
        ...
        for i in xrange(len(f)):
            ...
            if i>0:
                ...
                <td style="width:33.3%"><span class="label label-important">' +tmpsplit[2] + '</span></td>'
    except:
        table+='</table>'
    table+='</table>'
    return table
...
style_table='<style> .table.table-bordered.table-condensed td{text-align:center;}#table_general td{width:25%;}#table_samples td{width:50%;}#table_controls td{width:50%;}.table.table-bordered.table-condensed th{text-align:center;}</style>'
...
<tr><td>' +title+'</td><td><a target="_blank" href="http://genome.ucsc.edu/cgi-bin/hgGateway?db='+genome+'">'+genome+'</a></td><td>'+cellline+'</td>\
<td>'+alt_cellline+'</td></tr></table>'
...
download_btn='<div style="padding-top:20px;padding-bottom:25px;"><a target="_blank" href="'+pkfileurl+'"><button class="btn btn-primary btn-large" type="submit">Download Peak File <span class="icon-download icon-white"></span></button></a></div>'
